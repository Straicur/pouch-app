 ...
 * its own, separate authorization channel (product doc: "niezależny od
 * auth-tokena użytkownika"), so the key check already happened when the
 * signed link was generated. publicView() (and the file/thumbnail links
 * publicLink() mints alongside it) additionally uses a much longer-lived
 * signature (Part 9: "np. 24h") since it's meant to be opened by someone
 * with no account at all, not fetched immediately by the app that just
 * requested it.
 */
