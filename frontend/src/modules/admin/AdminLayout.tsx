...
import { ExceptionUuid, isApiError } from "../../lib/apiError";
...
const ADMIN_MODULES: { path: string; labelKey: string }[] = [
...
// area instead of every module re-probing — no role is exposed to JS at all
// (see adminApi's own docblock), so this still just renders whatever the
// storage report's 403 means, the same signal every module used individually
// before. StorageModule's own useGetStorageReportQuery() call below hits
    ...
    <SidebarLayout
      ...
      sidebar={
        <Sidebar>
          ...
          <SidebarBody>
            <SidebarSection>
              {ADMIN_MODULES.map(({ path, labelKey }) => (
