...
// no forced Roboto font/inline style override for the checkbox label variant
// (see button.tsx's comment for the general porting rationale).
...
export function Label({
  ...
  variant = "default",
  ...props
}: { className?: string; variant?: "default" | "checkbox" } & Omit<Headless.LabelProps, "as" | "className">) {
  return (
    <Headless.Label
      data-slot="label"
      {...props}
      className={clsx(
        className,
        "checkbox" === variant
          ? "text-sm text-zinc-700 select-none hover:text-zinc-950 data-disabled:opacity-50 dark:text-zinc-300 dark:hover:text-white"
