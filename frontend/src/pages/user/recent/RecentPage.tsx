...
// Ostatnio dodane itemy w całym drzewie, bez przeszukiwania kategorii — na
// wzór FavoritesPage: bez filtrów, tylko paginacja. Backend sortuje po
// createdAt DESC domyślnie (ItemRepository::findFilteredPage()), więc "od
// najnowszego" nie wymaga niczego dodatkowego poza brakiem filtrów.
const RECENT_PAGE_SIZE = 24;

export function RecentPage() {
  const { t } = useTranslation();
  const [page, setPage] = useState(1);
  const { data, isLoading, error } = useListItemsQuery({ page, pageSize: RECENT_PAGE_SIZE });

  const totalPages = undefined !== data ? Math.max(1, Math.ceil(data.total / data.pageSize)) : 1;

  return (
    <div className="flex flex-col gap-4">
      <Heading variant="page">{t("nav.recent")}</Heading>

      {isLoading && <LoadingIndicator />}
      {undefined !== error && <p className="text-red-600 dark:text-red-400">{t("items.fetchError")}</p>}
      {undefined !== data && 0 === data.items.length && <p>{t("recent.empty")}</p>}
