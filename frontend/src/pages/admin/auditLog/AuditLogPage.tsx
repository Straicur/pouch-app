...
// AuditLogEntry["action"]/["resourceType"] mirror the backend's snake_case
// values (see AuditLoggerInterface) — pl.ts's own keys stay camelCase
// (project convention), so this bridges the mismatches ("key_change" ->
// "keyChange", "technical_break" -> "technicalBreak") instead of
// introducing snake_case keys into the translation dictionary.
const auditActionLabelKey = (action: string): string => {
  return "key_change" === action ? "keyChange" : action;
};

const auditResourceLabelKey = (resourceType: string): string => {
  return "technical_break" === resourceType ? "technicalBreak" : resourceType;
};
        ...
        <TableBody>
          {entries?.map((entry) => (
            <TableRow key={entry.id}>
              ...
              <TableCell>
                {t(`admin.auditLog.resourceLabel.${auditResourceLabelKey(entry.resourceType)}`)} #{entry.resourceId}
              </TableCell>
